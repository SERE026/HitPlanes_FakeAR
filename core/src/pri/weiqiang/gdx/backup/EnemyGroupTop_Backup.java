 ...
 */
public class EnemyGroupTop_Backup extends Group {
	
	/*下边是生成右侧*/
	public EnemyGroupTop_Backup(AtlasRegion region) {
		super();
		int minX = 0;
		/*maxY注意与长宽要一致，否则会出现黑屏（无）比如把480改成800*/
        int maxX = (int) (480 - region.getRegionHeight());
        int tempX = MathUtils.random(minX, maxX);
		tempX = 0;
		
        for (int i = 0; i < 3; i++) {
            Image image = new Image(region);
            /*setX注意与长宽要一致*/
            image.setY(800 - image.getWidth());
